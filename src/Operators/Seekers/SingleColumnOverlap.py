...
class SingleColumnOverlap(Operator):
    def __init__(self, input_query_values, k=10):
        ...
        self.base_sql = ' $INIT$ ' + f'SELECT TableId FROM AllTables WHERE CellValue IN ($TOKENS$) $ADDITIONALS$ ' \
                        f'GROUP BY TableId, ColumnId ORDER BY COUNT(DISTINCT CellValue) DESC LIMIT $TOPK$' + ' $ENDING$ '
    ...
    def optimize(self, by, set_operation_type, create_executatable_query=False): # by, is an operator that we would like to optimize the sql based on
        ...
        if set_operation_type == 'set_intersection':
            if by.type == 'intersection':
                by.create_sql_query()
                # self.base_sql = self.base_sql.replace('$ADDITIONALS$', f' AND CellValue IN ({self.create_sql_where_condition_from_value_list(self.clean_value_collection(by.input))}) $ADDITIONALS$')
            ...
            if by.type == 'intersection':
                ...
                # self.base_sql = self.base_sql.replace('$INIT$', f' $INIT$ SELECT TableId FROM (SELECT * FROM ')
                # self.base_sql = self.base_sql.replace('$ENDING$', f' ) foo{random.randint(0, 1000)} GROUP BY TableId ORDER BY COUNT(*) DESC LIMIT {self.k} $ENDING$ ')
                # self.k = self.k + by.k
            elif by.type == 'multicolumnintersection' or by.type == 'quadrantapproximation':
