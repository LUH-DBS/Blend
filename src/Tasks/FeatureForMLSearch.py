...
def FeatureForMLSearch(query_dataset: pd.DataFrame, source_column_name: str, target_column_name: str, numerical_feature_column_name: str, k: int = 10) -> Plan:
    ...
    element_included = Correlation(query_dataset[source_column_name], query_dataset[target_column_name], k*1)
    plan.add('query_included', element_included, ['input'])
    element_excluded = Correlation(query_dataset[source_column_name], query_dataset[numerical_feature_column_name], k*1)
